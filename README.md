...
* **取得済みHTMLからの直接抽出**: `Extractor.ExtractText(ctx, io.Reader)` により、呼び出し側がすでに取得したレスポンスボディを再利用できます。Content-Type 判定などでHTTPレスポンスを取得済みの場合でも、同じURLへの重複リクエストを避けられます。
...
```text
go-web-exact/
├── extract/    # URLまたは取得済みHTMLからのコンテンツ抽出 (Extractor)
